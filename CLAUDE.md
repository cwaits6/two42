...
- **Every `createServiceClient()` query against an org-owned table filters on an `org_id` derived from an already-validated anchor** — the token row, the HMAC-validated group row, the caller's own RLS-scoped profile, or `app_request_org_id()` resolved through the cookie-bound request client (`await createClient()`, never the service client). Never a constant, never an org id read off the request body. The tenant root is the one exception: `organizations` carries no `org_id`, so a service-role read of it filters `id` against an org id that came from those same validated anchors (e.g. `lib/email/identity.ts` filters `.eq("id", orgId)` on an `orgId` its caller already holds) — never an id taken straight off the request body. Service-role clients carry `BYPASSRLS`, so the explicit filter *is* the tenant boundary on these surfaces. `schema_tenancy_lint.sql` cannot see it — but `scripts/check-service-role-org-scope.mjs` can, and does: it runs as the **blocking** `Service-role org_id guard` CI job. Run it locally before pushing, it takes under a second:

  ```bash
  npm run guard:tenancy
  ```

  Call sites and their org anchors: [`docs/security/service-role-inventory.md`](docs/security/service-role-inventory.md).

- **The guard has three tiers, and tier C catches code that never mentions the service client.** Full description in [`scripts/README.md`](scripts/README.md):
  - **Tier A** — the email fan-out chains, pinned by file + table. **No escape hatch**: an unscoped read there mails another org's members, and email cannot be recalled.
  - **Tier B** — every other chain rooted at a `createServiceClient()` binding. Escape hatch: an `// org-anchor: <reason>` comment on the chain, for the row that *resolves* the org (a signup token, a subscription token, an invite).
  - **Tier C** — chains rooted at a `SupabaseClient`-typed **parameter** of an exported `lib/` function. This is the one that surprises people: such a parameter is untyped as to privilege, so a caller can hand in a service client and every query in the helper becomes cross-tenant. It applies even when the function is only ever called with the cookie-bound request client today, and "RLS is the boundary" is **not** an answer to it — resolve a validated `orgId` in the caller and pass it in. No escape hatch for `.from()` chains; there is no exemption list. An `.rpc()` call on this tier that *is* the org resolver (`app_request_org_id()`) may carry a reasoned `// org-anchor:` marker, since it has no org to pass.
...
- **A `SECURITY DEFINER` function that writes an org-owned table is a tenant boundary of its own.** It bypasses RLS, so the org checks in its body replace the isolation policy — the `SECURITY DEFINER`/`org_id` lint check only proves the source mentions `org_id`. Resolve the org from a server-owned row (the `member_groups` row, `auth.uid()`'s profile, `app_request_org_id()`), **never from a caller parameter**; assert every other row carries that same `org_id` before writing; `set search_path = ''` and schema-qualify everything; `revoke execute … from public, anon` and grant only the roles that need it. Document the function in `docs/security/service-role-inventory.md` in the same PR, the same as a new service-role client. Reference implementation: `supabase/migrations/20260803010000_serving_signup_rpc.sql`.

Two rules in this section have **no lint behind them** and so need the most care:

- **Storage object keys are org-partitioned, and nothing checks it.** `storage.objects` has no `org_id` column, so the first path segment of the key is the tenant boundary: `<org_id>/<kind>/<entity_id>/<file>` (CWA-57). Build every key with `relObjectPath()` from `lib/storagePaths.ts` and upload/delete only through `uploadImage()` / `deleteImage()` in `lib/uploadImage.ts`, which resolve and prefix the org per call — a call site that reaches `supabase.storage.from(…)` directly can produce an un-prefixed key that no policy, lint, or type error catches. `schema_tenancy_lint.sql` is hard-scoped to `schemaname = 'public'` and never sees the `storage` schema; `check-service-role-org-scope.mjs` scans for `createServiceClient()`, not `.storage.from(`. Grep `.storage.from(` when reviewing a change that adds an upload surface, and add the matching `<kind>` to `StorageKind` plus a policy arm in the storage migration. Details: [`docs/security/tenancy-model.md`](docs/security/tenancy-model.md) ("Storage tenancy").
- **Edge-function service-role code enforces `org_id` itself.** The guard above also scans `supabase/functions/` (entry points and `_shared/`): every `.from()` and `.rpc()` chain there must carry an explicit `org_id` predicate, `org_id` on insert, or `org_id`/`_org_id` RPC argument, or a reasoned marker — only the `organizations` tenant root is exempt. It proves the predicate is present, not that the bound value is the right org. The cron edge functions run with the service key, which carries `BYPASSRLS`, so the isolation policies above do not constrain them at all. Iterate tenants with `listActiveOrgs()` / `forEachOrg()` from `supabase/functions/_shared/orgs.ts`, and put an explicit `.eq("org_id", …)` on **every** query and an explicit `org_id` on every insert. Nested PostgREST embeds are the one exception — they are reached by FK traversal from an already-org-filtered parent, and composite `(col, org_id)` FKs mean the traversal cannot leave the tenant — but that only holds if the parent is filtered. Neither `schema_tenancy_lint.sql` (which never sees TypeScript) nor `deno-test` (which does not exercise query bodies) can detect a wrong filter value: the guard fails a dropped `org_id` predicate, but a predicate bound to the wrong variable passes it. Review every `.from(` and `.rpc(` for the value it binds when a change lands here.
...
- Per-org branding (`organizations.branding`) is admin-supplied free text reaching CSS and RFC 5322 headers. `HEX` in `lib/contrast.ts`, the `CONTROL` control-character strip in `lib/branding.ts`, `PLAIN_NAME` in `lib/email/identity.ts`, and `SENDING_DOMAIN` in `lib/email/identity.ts` (the `org_email_domains.domain` gate on the `From:` address, verified-status-gated) are the injection boundary — not style choices. Do not relax them to support richer names or color formats; add a new validated key instead. `supabase/functions/_shared/branding.ts` mirrors all four byte-for-byte; a change lands on both sides. Emailed links are built on the one canonical host (`siteConfig.url`, or the `SITE_URL` secret in the edge functions), never on a per-org value — see [`docs/security/domains.md`](docs/security/domains.md).
