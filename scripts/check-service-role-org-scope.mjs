 ...
 * file in app/ and lib/ and enforces:
 *
 ...
 *            `// org-anchor: <why>` comment, or listed in KNOWN_ANCHORS.
...
const KNOWN_ANCHORS = [
  {
    ...
    table: "member_groups",
    reason: "the HMAC-validated group row is the org anchor for this signed link",
  },
  {
    file: "app/api/serving/link-action/route.ts",
    table: "profiles",
    reason: "profile org read unscoped so the cross-org pairing check can reject it explicitly",
  },
];

// Tier C is an over-approximation (any exported lib/ helper taking a
// SupabaseClient must scope its chains) and these pre-existing modules
// predate it: every caller passes the request-scoped AUTHENTICATED client,
// so RLS scopes their reads and the fail-closed org_id DEFAULT covers their
...
function outermostChainCall(callExpr) {
  let node = callExpr;
  for (;;) {
...
/** Does any expression subtree assign a property literally named org_id? */
function mentionsOrgIdProperty(node) {
  let found = false;
  function visit(n) {
    if (found) return;
    if (
...
function firstArgIsString(m, text) {
 ...
 */
function hasOrgPredicate(chain) {
 ...
 */
function markerState(sourceFile, chainTop) {
...
function scanFile(rel) {
  ...
  const usesServiceClient = /createServiceClient\s*\(/.test(text);
  const chains = [];

  // Pass 2: every `.from("<table>")` call, rooted leftward to decide whose
  // chain it is. Rooting at .from() handles all four syntactic positions
  // uniformly (await'd, bare Promise.all element, let-assigned, closure).
  (function collect(node) {
    if (
      ...
      if (root) {
        let kind = null;
        let fnName = null;
        if (serviceBindings.has(root.text)) {
          kind = "service";
...
// not be excused (or double-reported) by the Tier B pass below.
...
// Tier B (service chains) and Tier C (SupabaseClient-param chains in lib/).
const usedAnchors = new Set();
const usedTierCExemptions = new Set();
for (const c of allChains) {
  if (tierAChains.has(c) || hasOrgPredicate(c)) continue;
  if (c.kind === "tierC") {
    if (TIER_C_EXEMPT.has(c.file)) {
      usedTierCExemptions.add(c.file);
      continue;
    }
    fail(
      c.file,
      ...
      c.table,
      "C",
      `chain rooted at a SupabaseClient parameter${c.fnName ? ` of ${c.fnName}()` : ""} has no ` +
  ...
  if (c.marker === "bare") {
    fail(
      ...
      `// org-anchor: marker has no reason text. Every exception must be named — ` +
        `write why this chain is the org anchor.`
    );
    continue;
  }
  const known = KNOWN_ANCHORS.find((k) => k.file === c.file && k.table === c.table);
  if (known) {
    usedAnchors.add(known);
    continue;
  }
  fail(
    c.file,
    ...
    c.table,
    "B",
    `service-role chain on "${c.table}" has no org_id predicate. Add .eq("org_id", <validated anchor>) — ` +
      `or, if this chain IS the org anchor, a \`// org-anchor: <reason>\` comment above it and a row ` +
      `in ${INVENTORY}.`
  );
}

// Stale-allowlist checks: an entry that no longer matches an unscoped chain
// would otherwise rot into a permanent hole.
for (const k of KNOWN_ANCHORS) {
  if (!usedAnchors.has(k)) {
    fail(
      k.file,
      0,
      k.table,
      "B",
      `stale KNOWN_ANCHORS entry: no unscoped, unmarked chain on "${k.table}" exists here anymore. ` +
        `Delete the entry (and add an in-file // org-anchor: marker if the chain still exists).`
    );
  }
}
for (const f of TIER_C_EXEMPT) {
  if (!usedTierCExemptions.has(f)) {
    fail(
      f,
      0,
      "-",
      "C",
      `stale TIER_C_EXEMPT entry: no unscoped SupabaseClient-param chain exists here anymore. Delete the entry.`
    );
  }
}
    ...
    if (count !== 1) {
      fail(
        ...
        `expected the cross-org guard ${JSON.stringify(needle)} exactly once, found ${count}. ` +
          `The unscoped profiles read on this surface is only safe while this explicit rejection exists.`
      );
    }
  }
}
...
console.log(
  `tenancy guard OK — ` +
    `${scanTargets.length} files scanned; ` +
    `${serviceChainCount} service-role chains (tier A/B) + ${tierCChainCount} SupabaseClient-param chains (tier C) checked; ` +
    `${FANOUTS.length} fan-outs pinned; ${usedAnchors.size}/${KNOWN_ANCHORS.length} allowlist anchors and ` +
    `${usedTierCExemptions.size}/${TIER_C_EXEMPT.size} tier-C exemptions in use; ` +
