-- Stop seeding the dead site_name setting.
--
-- Nothing reads site_settings.site_name: the document title and every
-- email surface read organizations.branding.display_name, which
-- provisioning sets from _name. The seed row's only effect was a
-- "Site Name" field on /admin/settings that saved to a key no code path
-- consulted. Existing site_name rows are left in place (additive only); a
-- later cleanup may delete them once nothing seeds them.
--
-- provision_organization(): body copied verbatim from
-- 20260818000000_reserved_org_slugs.sql:14-140 (the live definition); ONLY
-- the ('site_name', '', true) seed row is removed. With it goes the only
-- is_public = true seed, so no provisioned setting is anon-readable.
...
-- Restated from 20260818000000:145-153, unchanged: create or replace keeps
